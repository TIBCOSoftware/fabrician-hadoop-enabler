...
public abstract class HadoopMBeanServerRef implements MXBeanServerRef {
    ...
    public abstract String getenbale_flag_var();
    public abstract String getjmx_baseport_var();
	public void init(Container container, Domain domain, ProcessWrapper process, RuntimeContext runtimeContext ) {
		...
		if (mBeanServerConnection == null) {
			...
			logger.log(Level.FINEST, "[HadoopMBeanServerRef] Enable flag name is [" + getenbale_flag_var() +"]");			
			if (rcvTrue(getenbale_flag_var(), runtimeContext)) {
			
				RuntimeContextVariable port = runtimeContext.getVariable(getjmx_baseport_var());
	...
	public boolean rcvTrue(String rcvname, RuntimeContext runtimeContext) throws IllegalArgumentException {
	    ...
	    if (TRUEVALS.contains(rcvvalue.toLowerCase())) result = true;
	    else
		    if (FALSEVALS.contains(rcvvalue.toLowerCase())) result = false;
		    else
	...
	public Object getAttribute(String bean, String attr) throws Exception {
		...
		if (mBeanServerConnection != null) {
			...
			try {
					attrValue = mBeanServerConnection.getAttribute(
							new ObjectName(bean), attrs[0]);
					if (attrs.length > 1) {
					...
					logger.fine(
							"[HadoopMBeanServerRef.getAttribute()] <"
							+ bean + "> <" + attr + "> <"
							+ ((Number) attrValue).doubleValue() + ">");
			} catch (Exception e) {
				// just log and ignore it while returning 0;
				logger.warning(
						"[HadoopMBeanServerRef.getAttribute()] <" + bean
						+ "> with attribute <" + attr
						+ "> threw an exception.");
				logger.warning(
						"[HadoopMBeanServerRef.getAttribute()] Exception: "
						+ e.getMessage());
				e.printStackTrace();
			...
			throw new IllegalStateException(
					"[HadoopMBeanServerRef.getAttribute()] mBeanServerConnectionhas not been initialized");
		}
	}

	public MBeanServerConnection getConnection() {
		logger.fine(
				"[HadoopMBeanServerRef.getAttribute()] mBeanServerConnection requested");
